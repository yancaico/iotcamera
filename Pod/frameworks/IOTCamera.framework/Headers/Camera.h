...
{
    TKAuthType_Password                  = 0,
    TKAuthType_Token                     = 1
};

typedef NS_ENUM(NSInteger, TKAudioSessionPlayMode) {
    TKAudioSessionPlayModeSpeaker = 0,
    TKAudioSessionPlayModeReceiver = 1
};
...
@property (nonatomic, copy)        NSString            *viewPwd;                           // 设备密码/token
...
@property (nonatomic, assign)      BOOL                isUseAuthKey;                       // 是否支持AuthKey
...
/**
 ...
 @param authType     设备验证类型
 @param bPlaybackMode 是否是录像回放模式
 */
- (void)TK_start:(NSInteger)channel viewAccount:(NSString *)viewAccount viewPassword:(NSString *)viewPassword authType:(TKAuthType)authType is_playback:(BOOL)bPlaybackMode;
...
#pragma mark - AV Filter
/**
视频滤镜

@param channel av通道
@param filterCMD 滤镜指令字串
@param isOn  开启/关闭滤镜
*/
- (void)TK_StartAVFilterWithChannel:(NSInteger)channel
                          filterCMD:(NSString *)filterCMD
                               isOn:(BOOL)isOn;
...
__attribute__((deprecated("deprecated")));
- (void)startSoundToDevice:(NSInteger)channel resend:(BOOL)isResend;
