...
//    IOTYPE_USER_GETIPAddress_RESP            = 0x702,
    ...
    IOTYPE_USER_IPCAM_SET_AUTHKEY_REQ           = 0x9014,
    IOTYPE_USER_IPCAM_SET_AUTHKEY_RESP          = 0x9015,
...
typedef struct
{
	...
	unsigned char reserved[4];
    unsigned char videoResoLution;        //设置录像： 1为高清，2为流畅
...
typedef struct{
    ...
    unsigned int VideoResoLution;///分辨率：0：为高清，1：为流畅
    unsigned char reserved[3];
}SMsgAVVideoResoLutionReq;
...
typedef struct
{
    ...
    char avservchannel; // 0子通道; 1主通道; otherwise 子通道（默认0）
...
** @struct SMsgAVIoctrlSetVsaasInfo
*/
typedef struct
{
	char VsaasUrl[256];
	char VsaasStoken[256];
} SMsgAVIoctrlSetVsaasInfoReq;

typedef struct
{
	int result;	// 0: success; otherwise: failed.
	unsigned char reserved[4];
}SMsgAVIoctrlSetVsaasInfoResp;
...
typedef struct
{
    ...
    unsigned char vender[32]; //制造商（需与OTA服务器上的厂商一致）
    unsigned int version; //当前版本号
    unsigned int free; //SD卡剩余空间
    unsigned int total; //SD卡总空间
    unsigned char region; //获取设备所在位置，0：中国区；1：非中国区
    unsigned char reserved[3];
...
typedef struct
{
    unsigned char isSupport; //0 不支持，1支持
    unsigned char reserved[4];
...
/*
IOTYPE_USER_IPCAM_SET_AUTHKEY_REQ = 0x9014;,
** @struct SMsgAVIoctrlSetAuthKeyReq
*/
typedef struct
{
    char oldauthkey[8];         // The old authkey code
    char newauthkey[8];         // The new authkey code
    unsigned char reserved[4];
}SMsgAVIoctrlSetAuthKeyReq;
 
/*
IOTYPE_USER_IPCAM_SET_AUTHKEY_RESP = 0x9015;,
** @struct SMsgAVIoctrlSetAuthKeyResp
*/
typedef struct
{
    int result; // 0: success; otherwise: failed.
    unsigned char reserved[4];
}SMsgAVIoctrlSetAuthKeyResp;
