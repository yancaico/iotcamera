...
typedef struct st_SInfoEx {
    ...
    uint8_t isUseAuthkey; //!< 0: Session connected without IOTC authkey, 1: Session connected with IOTC authkey
} st_SInfoEx_t;
 ...
 *                platform.
 *
 * \return On success, 0 is returned. On error, -46 is returned, it's an invalid argument.
 */
P2PAPI_API int32_t IOTC_Set_Max_Session_Number(int32_t nMaxSessionNum);
 ...
 *                the speciIOTC_Session_Check_Exxxa IOTC session has data or some error happens
 *                during the read data process
