 ...
 *            (4) This API has been deprecated due to security issue, please substitute it with
 *              IOTC_Device_loginEx to adopt authkey mechanism.
 */
...
*
* \details Enable device to collect system information and report to client in LAN mode or
*          device itself.
*
* \param pwd [in] User must input 8 bytes ASCII password.
* \param mode [in] ENABLE_LOCAL: allow device collect system information itself.
*                  ENABLE_REMOTE: allow client collect system information in LAN.
*
...
**/
P2PAPI_API int32_t IOTC_DebugTool_Initialize(char *pwd, DebugToolMode mode);

/**
* \brief Device fetch sdk information
*
* \details Device can collect local sdk information with NULL uid and NULL pwd if device init debug tool with ENABLE_LOCAL mode
*
...
* \return Return IOTC_ER_NoERROR if successfully.
* \return Error code if return value < 0
*            - #IOTC_ER_NOT_INITIALIZED The Debugtool module is not initialized yet
*            - #IOTC_ER_INVALID_ARG if buf is NULL pointer.
*            - #IOTC_ER_NOT_ENOUGH_MEMORY if buf_size less than actual data length.
*
* \attention (1) This API is a blocking function.
*
**/
P2PAPI_API int32_t IOTC_Fetch_DebugTool_Info(char *buf, int32_t buf_size);
